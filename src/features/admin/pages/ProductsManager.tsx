...
export function ProductsManager() {
  ...
  const profitMargin = useMemo(() => {
    ...
    return Math.round(((formData.price - formData.cost_per_item) / formData.price) * 100);
  }, [formData.price, formData.cost_per_item]);

  const progress = useMemo(() => {
    const fields = [
      formData.name_en && formData.category_id, // Basic
      formData.price > 0, // Pricing
      formData.has_one_size || formData.sizes.length > 0, // Sizes
      formData.colors.length > 0 || !formData.track_inventory, // Colors/Inventory
      formData.general_images.length > 0, // Images
      formData.description_en, // Details
    ];
  ...
  if (isLoading) {
    return <div className="flex h-[60vh] items-center justify-center"><Package className="h-8 w-8 animate-pulse text-primary" /></div>;
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl text-primary">{t.admin.products.title}</h1>
          <p className="text-muted-foreground">{filteredProducts.length} total products</p>
        </div>
        <Button onClick={() => handleOpenModal()} className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-gold">
          ...
          <table className="w-full text-sm text-left rtl:text-right">
            <thead className="text-[10px] uppercase tracking-widest text-muted-foreground border-b border-border bg-muted/30">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">{t.admin.products.category}</th>
                <th className="px-6 py-4 font-medium">{t.admin.products.price}</th>
                <th className="px-6 py-4 font-medium">{t.admin.products.status}</th>
                <th className="px-6 py-4 font-medium text-right rtl:text-left">{t.admin.products.actions}</th>
              </tr>
            </thead>
                ...
                <tr key={product.id} className="hover:bg-muted/20 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-muted-foreground uppercase tracking-widest">SKU: {product.sku || "N/A"}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-muted-foreground">{product.categories?.name_en || "Uncategorized"}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right rtl:text-left">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" onClick={() => handleOpenModal(product)} className="h-8 w-8 text-primary">
                        <Edit2 className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => { setProductToDelete(product); setIsDeleteConfirmOpen(true); }} className="h-8 w-8 text-destructive">
      ...
      {/* --- Main Modal --- */}
      <Dialog open={isModalOpen} onOpenChange={(open) => !open && handleCloseModalAttempt()}>
        <DialogContent className="max-w-5xl max-h-[92vh] overflow-hidden flex flex-col p-0 bg-card border-border shadow-2xl">
          <DialogHeader className="p-6 border-b border-border bg-background/50">
            <div className="flex items-center justify-between">
              <div>
                <DialogTitle className="font-display text-2xl text-primary">
                  {formData.id ? t.admin.products.modal.editTitle : t.admin.products.modal.addTitle}
                </DialogTitle>
                <div className="flex items-center gap-4 mt-2">
                  <div className="h-1.5 w-32 bg-muted rounded-full overflow-hidden">
                    <div className="h-full bg-primary transition-all duration-500" style={{ width: `${progress}%` }} />
                  </div>
                  <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{progress}% Complete</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button variant="outline" onClick={handleCloseModalAttempt} className="bg-background border-border">
          ...
          <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
            <Accordion type="multiple" defaultValue={["basic"]} className="space-y-4">
              ...
              <AccordionItem value="basic" className="border border-border rounded-lg overflow-hidden bg-background/30">
                <AccordionTrigger className="px-4 py-3 hover:no-underline">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-sm uppercase tracking-widest">{t.admin.products.modal.sections.basic}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-4 pb-4 space-y-6 pt-2">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.nameEn}</Label>
                      <Input value={formData.name_en} onChange={(e) => handleFormChange({ name_en: e.target.value })} className="bg-background border-border" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.nameAr}</Label>
                      <Input dir="rtl" value={formData.name_ar} onChange={(e) => handleFormChange({ name_ar: e.target.value })} className="bg-background border-border text-right" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.nameTr}</Label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Select value={formData.category_id} onValueChange={(val) => handleFormChange({ category_id: val })}>
              ...
              {/* 2. Pricing & SKU */}
              <AccordionItem value="pricing" className="border border-border rounded-lg overflow-hidden bg-background/30">
                <AccordionTrigger className="px-4 py-3 hover:no-underline">
                  <div className="flex items-center gap-3">
                    <DollarSign className="h-4 w-4 text-primary" />
                    <span className="font-bold text-sm uppercase tracking-widest">{t.admin.products.modal.sections.pricing}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-4 pb-4 pt-2">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.price} (USD)</Label>
                      <div className="relative">
                        <DollarSign className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3 w-3 text-muted-foreground" />
                        <Input type="number" value={formData.price} onChange={(e) => handleFormChange({ price: Number(e.target.value) })} className="pl-8 bg-background border-border" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.compareAt}</Label>
                      <Input type="number" value={formData.compare_at_price} onChange={(e) => handleFormChange({ compare_at_price: Number(e.target.value) })} className="bg-background border-border" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.costPerItem}</Label>
                      <Input type="number" value={formData.cost_per_item} onChange={(e) => handleFormChange({ cost_per_item: Number(e.target.value) })} className="bg-background border-border" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.margin}</Label>
              ...
              {/* 3. Inventory & Sizes */}
              <AccordionItem value="inventory" className="border border-border rounded-lg overflow-hidden bg-background/30">
                <AccordionTrigger className="px-4 py-3 hover:no-underline">
                  <div className="flex items-center gap-3">
                    <Layers className="h-4 w-4 text-primary" />
                    <span className="font-bold text-sm uppercase tracking-widest">{t.admin.products.modal.sections.inventory}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-4 pb-4 space-y-6 pt-2">
                  <div className="flex items-center justify-between p-4 rounded-lg bg-muted/10 border border-border/50">
                    <div className="space-y-0.5">
                      <Label className="text-sm font-bold uppercase tracking-widest">{t.admin.products.modal.trackInventory}</Label>
                      <p className="text-[10px] text-muted-foreground">Keep record of your stock levels</p>
                    </div>
                    <Switch checked={formData.track_inventory} onCheckedChange={(val) => handleFormChange({ track_inventory: val })} />
                  </div>
                  ...
                  {formData.track_inventory && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                      {!formData.colors.length && (
                        <div className="space-y-2">
                          <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.stock}</Label>
                          <Input type="number" value={formData.total_stock} onChange={(e) => handleFormChange({ total_stock: Number(e.target.value) })} className="bg-background border-border" />
                        </div>
                      )}
                      <div className="space-y-2">
                        <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.lowStock}</Label>
                        <Input type="number" value={formData.low_stock_threshold} onChange={(e) => handleFormChange({ low_stock_threshold: Number(e.target.value) })} className="bg-background border-border" />
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-4 pt-4 border-t border-border/50">
                    <div className="flex items-center justify-between">
                      <Label className="text-sm font-bold uppercase tracking-widest">{t.admin.products.modal.sizes}</Label>
                      <div className="flex items-center gap-2">
                        <Label className="text-[10px] text-muted-foreground uppercase">{t.admin.products.modal.oneSize}</Label>
                        <Checkbox checked={formData.has_one_size} onCheckedChange={(val) => handleFormChange({ has_one_size: val as boolean })} />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.sizeGuide}</Label>
                      <Input value={formData.size_guide} onChange={(e) => handleFormChange({ size_guide: e.target.value })} placeholder="URL or descriptive text" className="bg-background border-border" />
                    </div>
                  </div>
                </AccordionContent>
              </AccordionItem>
              ...
              {/* 4. Color Variants */}
              <AccordionItem value="colors" className="border border-border rounded-lg overflow-hidden bg-background/30">
                <AccordionTrigger className="px-4 py-3 hover:no-underline">
                  <div className="flex items-center gap-3">
                    <Palette className="h-4 w-4 text-primary" />
                    <span className="font-bold text-sm uppercase tracking-widest">{t.admin.products.modal.sections.colors}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-4 pb-4 space-y-4 pt-2">
                  <div className="space-y-4">
                    {formData.colors.map((color, i) => (
                      <Card key={color.id} className="bg-background border-border relative group overflow-hidden">
                        <div className="p-4 space-y-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <div className="h-6 w-6 rounded-full border border-border shadow-sm" style={{ backgroundColor: color.hex }} />
                              <span className="font-bold text-xs uppercase tracking-widest">{color.name_en || "Unnamed Color"}</span>
                              {color.is_main && <Badge className="text-[8px] bg-primary/20 text-primary">MAIN</Badge>}
                            </div>
                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => moveColor(i, "up")} disabled={i === 0} className="text-muted-foreground hover:text-primary disabled:opacity-20"><ChevronUp className="h-4 w-4" /></button>
                              <button onClick={() => moveColor(i, "down")} disabled={i === formData.colors.length - 1} className="text-muted-foreground hover:text-primary disabled:opacity-20"><ChevronDown className="h-4 w-4" /></button>
                              <Button variant="ghost" size="icon" onClick={() => handleFormChange({ colors: formData.colors.filter(c => c.id !== color.id) })} className="h-7 w-7 text-destructive"><Trash2 className="h-3.5 w-3.5" /></Button>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                              <div className="space-y-1.5">
                                <Label className="text-[8px] uppercase font-bold text-muted-foreground">Name EN</Label>
                                <Input value={color.name_en} onChange={(e) => updateColor(color.id, { name_en: e.target.value })} className="h-8 text-xs bg-muted/30" />
                              </div>
                              <div className="space-y-1.5">
                                <Label className="text-[8px] uppercase font-bold text-muted-foreground">Name AR</Label>
                                <Input dir="rtl" value={color.name_ar} onChange={(e) => updateColor(color.id, { name_ar: e.target.value })} className="h-8 text-xs bg-muted/30 text-right" />
                              </div>
                              <div className="space-y-1.5">
                                <Label className="text-[8px] uppercase font-bold text-muted-foreground">Name TR</Label>
                                <Input value={color.name_tr} onChange={(e) => updateColor(color.id, { name_tr: e.target.value })} className="h-8 text-xs bg-muted/30" />
                              </div>
                            </div>
                            <div className="space-y-1.5">
                              <Label className="text-[8px] uppercase font-bold text-muted-foreground">Hex Color</Label>
                              <div className="flex gap-2">
                                <Input type="color" value={color.hex} onChange={(e) => updateColor(color.id, { hex: e.target.value })} className="h-8 w-12 p-0 bg-transparent border-none" />
                                <Input value={color.hex} onChange={(e) => updateColor(color.id, { hex: e.target.value })} className="h-8 text-xs font-mono bg-muted/30" />
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                            <div className="space-y-1.5">
                              <Label className="text-[8px] uppercase font-bold text-muted-foreground">{t.admin.products.modal.stock}</Label>
                    ...
                    <Button variant="outline" className="w-full border-dashed border-primary/40 text-primary hover:bg-primary/5 h-10" onClick={addColorVariant}>
              ...
              {/* 5. Images */}
              <AccordionItem value="images" className="border border-border rounded-lg overflow-hidden bg-background/30">
                <AccordionTrigger className="px-4 py-3 hover:no-underline">
                  <div className="flex items-center gap-3">
                    <ImageIcon className="h-4 w-4 text-primary" />
                    <span className="font-bold text-sm uppercase tracking-widest">{t.admin.products.modal.sections.images}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-4 pb-4 space-y-4 pt-2">
              ...
              <AccordionItem value="details" className="border border-border rounded-lg overflow-hidden bg-background/30">
                <AccordionTrigger className="px-4 py-3 hover:no-underline">
                  <div className="flex items-center gap-3">
                    <AlertCircle className="h-4 w-4 text-primary" />
                    <span className="font-bold text-sm uppercase tracking-widest">{t.admin.products.modal.sections.details}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-4 pb-4 space-y-6 pt-2">
                  <Tabs defaultValue="en" className="w-full">
                    <TabsList className="bg-muted/50 border border-border mb-4">
                      <TabsTrigger value="en" className="text-[10px] uppercase font-bold px-4 tracking-widest">English</TabsTrigger>
              ...
              <AccordionItem value="additional" className="border border-border rounded-lg overflow-hidden bg-background/30">
                <AccordionTrigger className="px-4 py-3 hover:no-underline">
                  <div className="flex items-center gap-3">
                    <Settings className="h-4 w-4 text-primary" />
                    <span className="font-bold text-sm uppercase tracking-widest">{t.admin.products.modal.sections.additional}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-4 pb-4 space-y-6 pt-2">
                  <div className="flex items-center justify-between p-4 rounded-lg bg-primary/5 border border-primary/10">
                    ...
                    <Switch checked={formData.is_featured} onCheckedChange={(val) => handleFormChange({ is_featured: val })} />
                  </div>

                  <div className="space-y-2">
                    <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">{t.admin.products.modal.relatedProducts}</Label>
                    <Select onValueChange={(val) => !formData.related_products.includes(val) && handleFormChange({ related_products: [...formData.related_products, val] })}>
                      <SelectTrigger className="bg-background border-border">
      ...
      <Dialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
        <DialogContent className="bg-card border-border">
          <DialogHeader><DialogTitle className="text-xl text-primary font-display">{t.admin.products.confirmDelete}</DialogTitle></DialogHeader>
          <div className="py-4 text-muted-foreground">
            {productToDelete && (
              <p>Are you sure you want to delete <span className="text-foreground font-bold">
                {locale === "ar" ? productToDelete.name_ar : locale === "tr" ? productToDelete.name_tr : productToDelete.name_en}
      ...
      <Dialog open={isUnsavedConfirmOpen} onOpenChange={setIsUnsavedConfirmOpen}>
        <DialogContent className="bg-card border-border">
          <DialogHeader><DialogTitle className="text-xl text-primary font-display">Unsaved Changes</DialogTitle></DialogHeader>
          <p className="py-4 text-muted-foreground">You have unsaved changes that will be lost if you close this modal. Are you sure?</p>
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setIsUnsavedConfirmOpen(false)} className="bg-background border-border">Keep Editing</Button>
            <Button variant="destructive" onClick={() => { setIsUnsavedConfirmOpen(false); setIsUnsaved(false); setIsModalOpen(false); }}>Discard & Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
