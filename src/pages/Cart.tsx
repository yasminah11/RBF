  ...
  return (
    <div className="container-luxury py-10 md:py-16 min-h-[70vh]">
      <div className="text-center mb-10 px-4">
        <p className="text-[10px] md:text-xs uppercase tracking-[0.4em] text-primary mb-3">Your Selection</p>
        <h1 className="font-display text-3xl md:text-5xl text-cream">{t.cart.title}</h1>
      ...
      {items.length === 0 ? (
        <motion.div 
          ...
          className="text-center py-20 px-4 bg-card/20 border border-dashed border-border/20 max-w-2xl mx-auto rounded-lg"
        >
          <div className="bg-primary/10 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6">
            <ShoppingBag className="text-primary w-8 h-8" />
          </div>
          <p className="text-muted-foreground mb-8 text-lg italic">{t.cart.empty}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 px-2 md:px-0">
          ...
          <div className="lg:col-span-8 space-y-6">
                  ...
                  >
                    ...
                    <div className="col-span-6 flex gap-4 md:gap-6 w-full">
                      ...
                      <div className="flex flex-col justify-center text-start">
                        <h3 className="font-display text-lg md:text-2xl text-cream mb-1">{i.name}</h3>
                        <p className="text-[10px] md:text-xs uppercase tracking-widest text-primary font-medium">{i.variantLabel}</p>
                        <p className="text-muted-foreground text-sm mt-2 md:hidden">{formatPrice(i.price)}</p>
                      </div>
                    </div>
                        ...
                        <div className="w-12 flex items-center justify-center border-x border-border/30 overflow-hidden relative">
                          <AnimatePresence mode="wait">
                            <motion.span 
                              ...
                              className="text-xs font-medium text-cream block"
                            >
                              {i.quantity}
                            </motion.span>
                          </AnimatePresence>
            ...
            <div className="pt-4 flex justify-between md:hidden">
              <span className="text-muted-foreground uppercase text-[10px] tracking-widest">Total Selection</span>
              <motion.span 
                key={total}
          ...
          <div className="lg:col-span-4 mt-8 lg:mt-0">
            <div className="bg-card/60 backdrop-blur-sm border border-border/20 p-6 md:p-8 sticky top-24">
              <h2 className="font-display text-2xl md:text-3xl text-cream mb-8 border-b border-border/10 pb-4">{t.cart.subtotal}</h2>
              ...
              <div className="border-t border-border/20 pt-6 flex justify-between items-end mb-10">
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-1">Total Amount</span>
                  <motion.span 
                    key={total}
                    initial={{ y: 5, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    className="text-primary text-2xl md:text-3xl font-light"
                  >
                    {formatPrice(total)}
                  </motion.span>
                </div>
              ...
              <Link 
                to="/checkout"
                className="group relative block w-full bg-primary text-primary-foreground py-5 text-[10px] md:text-xs uppercase tracking-[0.3em] font-bold overflow-hidden transition-all hover:shadow-gold text-center"
              >
